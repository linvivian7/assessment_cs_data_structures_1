def string_compare(s1, s2):
    ...
    Put runtime here:
    -----------------
    [ n=# per string  O(N); one operation each on each string; but coefficient is dropped ]
...
def has_exotic_animals(animals):
    ...
    Put runtime here:
    -----------------
    [ O(N) worst case, there are no exotic animals so have to iteratre the entire list ]
...
def sum_zero_1(numbers):
    ...
    Put runtime here:
    -----------------
    [ O(N); finding numbers in sets is an O(1) operation * N inputs; O(2N) --> O(2N) ]
...
def sum_zero_2(numbers):
    ...
    Put runtime here:
    -----------------
    [ O(N^2); a loop of length N within each operation for N equates to O(1) * N^2   ]
...
def sum_zero_3(numbers):
    ...
    Put runtime here:
    -----------------
    [ O(N^3); similar reasoning as above; however, the act of checking for existence in a list is an O(N) operation so now the runtime is
      O(N) * N^2 ]
